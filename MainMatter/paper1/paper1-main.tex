...
\blfootnote{The contents of this chapter are under review at A FANCY JOURNAL, for which a preprint is available on arXiv~\parencite{Einstein1906}.}
...
\end{quote}
which loads the dissertation template. The template is based on the \LaTeX{} \texttt{book} document class and stored in \texttt{dissertation.cls}. The document class accepts several comma-separated options. By default, hyperlinks are shown in black, but this can be changed. Which is convenient when reading the dissertation on a computer, but can be expensive when printing. 

\emph{these options don't work atm}
They can be turned black with the \texttt{print} option. This will also turn the headers dark gray instead of cyan. Moreover, it will add a 3~mm bleed around the page including crop marks. This will help the printer with the thumb indices, since they run right up to the page borders. Finally, the \texttt{nativefonts} option can be used to override the automatic font selection (see below).
...
The corporate colors of the Utrecht University are red, black and yellow, available, respectively, via \texttt{\textbackslash color\{{\color{uu-red}uu-red}\}}, \texttt{\textbackslash color\{{\color{uu-black}uu-black}\}} (which differs slightly from the default \texttt{black}) and \texttt{\textbackslash color\{\color{uu-yellow}uu-yellow\}}. Apart from these three, the house style defines the basic colors
\begin{itemize}
%% Reduce the separation between the items, since this is just a list of words.
\itemsep 0pt
\parskip 0pt
\item \texttt{\color{uu-creme}uu-creme},
\item \texttt{\color{uu-orange}uu-orange},
\item \texttt{\color{uu-bordeaux}uu-bordeaux},
\item \texttt{\color{uu-brown}uu-brown},
\item \texttt{\color{uu-green}uu-green},
\item \texttt{\color{uu-blue}uu-blue},
\item \texttt{\color{uu-darkblue}uu-darkblue} and
\item \texttt{\color{uu-purple}uu-purple} 
\end{itemize}